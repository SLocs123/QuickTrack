...
def train(params):
    ...
    weightDisp = params[3]
    weightCol = params[4]
    weightShape = params[5]
    weightClass = params[6]
    ...
    lossValue = 0  # must find a good loss function ---------------------------------------------------------------------------------------

    return lossValue


def gradient(params, delta):
    grads = []
    for pos, item in enumerate(params):
        upper = params
        upper[pos] = item + delta
        lower = params
        lower[pos] = item - delta

        grads[pos] = (train(upper) - train(lower))/(2*delta)
    return grads
...
step = 5
delta = 0.01
params = []  # Define the starting parameters of the model
lastValue = 0
tol = 0.01

for i in range(interations):
    grad = gradient(params, delta)
    for num, line in enumerate(params):
        diff = -step * line
        params[num] += diff
    value = train(params)
    print('Value: ', value, ' ------- ', 'Params: ', params)
    if lastValue:
