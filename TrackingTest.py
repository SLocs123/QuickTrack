        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if frame == 1:
                    ...
                    maxDisp = [200, 100]  # [x, y]
                    maxColDif = 100  # max delta e/euclidean difference
                    maxHeightDif = 100

                    gradDisp = [1 / maxDisp[0], 1 / maxDisp[1]]
                    gradCol = 1/maxColDif
                    gradHeight = 1/maxHeightDif

                    # 1,2,3,4,5,6
                    # weightDisp, weightCol, Height, weightClass
                    weights = [15, 3, 3, 2]
                    ...
                    for det_obj in detList:
                        ...
                        for index, obj in enumerate(objects):
                            ...
                            # colour
                            delta_e = delta_e_cie2000(obj_color, obj['colour'])  # using the 1976 colour difference standard for performance
                            # delta_e = deltaE_cie76(obj_color, obj['colour'])
                            conf.append(-gradCol * delta_e + 1 if delta_e < maxColDif else 0)

                            # Height
                            confHeight = (-gradHeight * np.absolute((obj['shape'] - height)) + 1) if np.absolute(obj['shape']-height) < maxHeightDif else 0
                            conf.append(confHeight)
                ...
                file = open('weights 15, 3, 3, 2domCIE2000.txt', 'a')
                for obj in objects:
                    to_write = " ".join(str(a) for a in [
                        frame,
                        obj['id'],
                        names[obj['cls']],
                ...
                for obj in objects:
                    ...
                    plot_one_box(xyxy, im0, label=label, color=colors[obj['cls']], line_thickness=1)
